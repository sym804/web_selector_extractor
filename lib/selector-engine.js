...
const SelectorEngine = (() => {
  ...
  // Playwright 코드 생성. frame 이 주어지면 frame_locator 로 감싼다.
  function toPlaywright(selectorResult, frame) {
    const root = (frame && frame.inFrame && frame.frameSelector)
      ? `page.frame_locator("${escAttr(frame.frameSelector)}")`
      : 'page';
  ...
  // Playwright 액션 코드 생성
  function toPlaywrightAction(selectorResult, action = 'click', frame) {
    const locator = toPlaywright(selectorResult, frame);
    switch (action) {
  ...
  return { extract, toPlaywright, toPlaywrightAction, getElementInfo, getFrameContext };
})();
